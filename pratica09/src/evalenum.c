...
void heapify(int * A, int l, int r, sortperf_t *s){
    ...
    while(j <= r){
        // inccmp(s, 2); // While and if statements
        if(j < r){
            inccmp(s, 2);
            if(A[j] < A[j + 1]) j ++;
            if(key >= A[j]) break;
...
//insertion sort -- I'm not sure if it's correct
void insertionSort(int v[], int l, int r, sortperf_t * s) { 
    inccalls(s, 1);
    int i, j, key;
    // Aumentando o subarray ordenado em uma unidade
    for (i = l + 1; i <= r; i++){
        incmove(s, 1);
        key = v[i];
        j = i - 1;
...
void partition3(int *   A, int l, int r, int *i, int *j, sortperf_t *s) {
    ...
    *i = l; *j = r;
    ...
    do{
        ...
        // Se ainda nao se cruzaram
        if(*i <= *j){
            swap(&A[*i], &A[*j], s);
            (*i)++;(*j)--;
        }

        // inccmp(s, 1); // while condition
    }while(*i <= *j);
...
void quickSortIns(int * A, int l, int r, sortperf_t *s) { 
    inccalls(s, 1);
    // inccmp(s, 1);
    if(r - l + 1 <= 50) return insertionSort(A, l, r, s); 
