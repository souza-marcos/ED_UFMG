...
Matrix::Matrix(std::size_t rows, std::size_t cols, std::vector<double> values, int id) :
    rows_(rows), cols_(cols), m_(values), row_based_(true), _id(id) {
      ESCREVEMEMLOG((long int)(&m_), sizeof(double) *  m_.size(), _id);
}
...
double Matrix::at(std::size_t row, std::size_t col) const {
  LEMEMLOG((long int)m_[GetIndex(row, col)], sizeof(double), this->_id);
...
Matrix Matrix::LUDecompositionByGE(int* p_swap_count,
  ...
  *p_swap_count = 0;
  ...
  Matrix m(n, n, a, (int)1);
  ESCREVEMEMLOG((long int)(&m.m_), a.size() * sizeof(double), m._id);
  ...
  for (std::size_t k = 0; k < n - 1; k++) {
    ...
    for (std::size_t i = k + 1; i < n; i++) {
      ...
      LEMEMLOG((long int)(&m.m_[ik]), sizeof(double), m._id);
      ESCREVEMEMLOG((long int)(&m.m_[ik]), sizeof(double), m._id);
    }
    for (std::size_t i = k + 1; i < n; i++) {
      ...
      for (std::size_t j = k + 1; j < n; j++) {
        ...
        LEMEMLOG((long int)(&m.m_[ij]), sizeof(double), m._id);
        ESCREVEMEMLOG((long int)(&m.m_[ij]), sizeof(double), m._id);
      }
    }
  }
